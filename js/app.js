...
function renderResults(fastest, fewest) {
  ...
  const cards = [];
  cards.push(routeCardHTML(fastest, sameRoute ? '最短ルート' : '最短時間', sameRoute ? 'fastest' : 'fastest'));
  if (fewest && !sameRoute) cards.push(routeCardHTML(fewest, '乗換少なめ', 'fewest'));
  list.innerHTML = cards.join('');
}

function routeCardHTML(route, tagLabel, tagClass) {
